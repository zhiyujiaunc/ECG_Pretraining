...
def setup_parser(parser: argparse.ArgumentParser) -> None:
    ...
    # dataset settings
    parser.add_argument('--modality', nargs='+', default=['eeg'])
    ...
    parser.add_argument("--skip_periodic_eval", action='store_true')
    ...
    parser.add_argument("--ft_channel", nargs='+', default=['default'])
    parser.add_argument('--ft_n_channel', nargs='+', type=int, default=[1])
...
class Trainer():
    ...
    def train_epoch(self, recon=False):
        ...
        for i, (data, label) in enumerate(tqdm(self.train_loader, disable=self.args.disable_tqdm)):
            self.global_step += 1
    ...
    def main(self, recon=False, TB_name='TrainAcc', tt_epoch=50):
        ...
        for epoch in range(tt_epoch):
            ...
            if self.args.save_every_epoch and (epoch + 1) % self.args.save_every_epoch == 0:
                self._save_model()

            if self.args.skip_periodic_eval:
                continue
